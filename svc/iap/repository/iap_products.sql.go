...
VALUES (
    $1,
    $2
) RETURNING id, price_in_sao, updated_at, created_at
`
...
func (q *Queries) CreateIapProduct(ctx context.Context, arg CreateIapProductParams) (IapProduct, error) {
	row := q.queryRow(ctx, q.createIapProductStmt, createIapProduct, arg.ID, arg.PriceInSao)
	var i IapProduct
	err := row.Scan(
		&i.ID,
		&i.PriceInSao,
...
SELECT id, price_in_sao, updated_at, created_at FROM iap_products
WHERE id = $1
`
